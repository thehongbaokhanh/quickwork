...
function extendRef(ref3, extend, { enumerable = false, unwrap = true } = {}) {
  for (const [key, value] of Object.entries(extend)) {
    ...
    if (isRef(value) && unwrap) Object.defineProperty(ref3, key, {
      get() {
        return value.value;
      },
      set(v) {
    ...
    else Object.defineProperty(ref3, key, {
      value,
      enumerable
    });
  }
...
import { Fragment, TransitionGroup, computed as computed2, customRef as customRef2, defineComponent, getCurrentInstance as getCurrentInstance2, getCurrentScope as getCurrentScope2, h, hasInjectionContext as hasInjectionContext2, inject as inject2, isReadonly, isRef as isRef2, markRaw, nextTick as nextTick2, onBeforeUpdate, onMounted as onMounted2, onUpdated, reactive as reactive2, readonly as readonly2, ref as ref2, shallowReactive, shallowReadonly as shallowReadonly2, shallowRef as shallowRef2, toRaw, toValue as toValue2, unref as unref2, watch as watch2, watchEffect as watchEffect2 } from "vue";
function computedAsync(evaluationCallback, initialState, optionsOrRef) {
  var _globalThis$reportErr;
  let options;
  if (isRef2(optionsOrRef)) options = { evaluating: optionsOrRef };
...
function computedInject(key, options, defaultSource, treatDefaultAsFactory) {
  ...
  if (defaultSource) source = inject2(key, defaultSource);
  if (treatDefaultAsFactory) source = inject2(key, defaultSource, treatDefaultAsFactory);
  if (typeof options === "function") return computed2((oldValue) => options(source, oldValue));
  else return computed2({
...
function useMutationObserver(target, callback, options = {}) {
  ...
  const stopWatch = watch2(computed2(() => {
    const items = toArray(toValue2(target)).map(unrefElement).filter(notNullish);
...
function useRafFn(fn, options = {}) {
  ...
  tryOnScopeDispose(pause);
  return {
    isActive: shallowReadonly2(isActive),
    pause,
    resume
  };
}
function useAnimate(target, keyframes, options) {
  ...
  watch2(() => keyframes, (value) => {
    if (animate.value) {
      ...
      if (targetEl) animate.value.effect = new KeyframeEffect(targetEl, toValue2(value), animateOptions);
    }
  }, { deep: true });
  ...
  function update(init) {
    ...
    if (!animate.value) animate.value = el.animate(toValue2(keyframes), animateOptions);
...
function useAsyncQueue(tasks, options) {
  ...
  const result = reactive2(Array.from(Array.from({ length: tasks.length }), () => ({
...
function useAsyncState(promise, initialState, options) {
  ...
  async function execute(delay2 = 0, ...args) {
    ...
    if (resetOnExecute) state.value = toValue2(initialState);
    error.value = void 0;
    isReady.value = false;
...
function useBase64(target, options) {
  ...
  if (isRef2(target) || typeof target === "function") watch2(target, execute, { immediate: true });
  else execute();
  return {
    base64,
    promise,
...
function useBluetooth(options) {
  ...
  return {
    isSupported,
    isConnected: shallowReadonly2(isConnected),
    device,
    requestDevice,
    server,
    error
...
function useMediaQuery(query, options = {}) {
  ...
  watchEffect2(() => {
    if (ssrSupport.value) {
      ...
      matches.value = toValue2(query).split(",").some((queryString) => {
...
function useBrowserLocation(options = {}) {
  ...
  const refs = Object.fromEntries(WRITABLE_PROPERTIES.map((key) => [key, ref2()]));
  for (const [key, ref3] of objectEntries(refs)) watch2(ref3, (value) => {
...
function useClipboard(options = {}) {
  ...
  const copyPending = shallowRef2(false);
  ...
  async function copy(value) {
    const resolvedValue = value !== null && value !== void 0 ? value : toValue2(source);
  ...
  return {
    copyPending: shallowReadonly2(copyPending),
    isSupported,
    text: shallowReadonly2(text),
    copied: shallowReadonly2(copied),
    copy
  };
}
...
function useCloned(source, options = {}) {
  ...
  if (!manual && (isRef2(source) || typeof source === "function")) watch2(source, sync, {
    ...options,
    deep,
    immediate
  });
...
function useStorage(key, defaults2, storage, options = {}) {
  ...
  function update(event) {
    ...
    try {
      ...
      if (event) nextTick2(resumeWatch);
      else resumeWatch();
    }
  }
...
function useCountdown(initialCountdown, options = {}) {
  ...
  const reset = (countdown) => {
    var _toValue;
    remaining.value = (_toValue = toValue2(countdown)) !== null && _toValue !== void 0 ? _toValue : toValue2(initialCountdown);
  };
  const stop = () => {
    controls.pause();
    reset();
...
function useCssSupports(...args) {
  let options = {};
  if (typeof toValue2(args.at(-1)) === "object") options = args.pop();
  const [prop, value] = args;
  const { window: window2 = defaultWindow, ssrValue = false } = options;
  const isMounted = useMounted();
  return { isSupported: computed2(() => {
    if (!isClient || !isMounted.value) return ssrValue;
    return args.length === 2 ? window2 === null || window2 === void 0 ? void 0 : window2.CSS.supports(toValue2(prop), toValue2(value)) : window2 === null || window2 === void 0 ? void 0 : window2.CSS.supports(toValue2(prop));
  }) };
}
function useCssVar(prop, target, options = {}) {
  ...
  watch2([elRef, () => toValue2(prop)], (_, old) => {
...
function useCycleList(list, options) {
  ...
  function getInitialValue() {
    ...
    return (_toValue = toValue2((_options$initialValue = options === null || options === void 0 ? void 0 : options.initialValue) !== null && _options$initialValue !== void 0 ? _options$initialValue : toValue2(list)[0])) !== null && _toValue !== void 0 ? _toValue : void 0;
  }
...
function useManualRefHistory(source, options = {}) {
  ...
  return {
    ...
    redoStack,
    last,
    history: computed2(() => [last.value, ...undoStack.value]),
    canUndo: computed2(() => undoStack.value.length > 0),
    canRedo: computed2(() => redoStack.value.length > 0),
    clear,
    commit,
    reset,
    undo,
...
function useDeviceMotion(options = {}) {
  ...
  const acceleration = ref2({
    x: null,
    y: null,
    z: null
  });
  const rotationRate = ref2({
    alpha: null,
    beta: null,
    gamma: null
  });
...
function useDevicePixelRatio(options = {}) {
  ...
  return {
    pixelRatio: shallowReadonly2(pixelRatio),
    stop
  };
}
...
function useDisplayMedia(options = {}) {
  ...
  const enabled = shallowRef2((_options$enabled = options.enabled) !== null && _options$enabled !== void 0 ? _options$enabled : false);
...
function useDocumentVisibility(options = {}) {
  const { document: document2 = defaultDocument } = options;
  if (!document2) return shallowRef2("visible");
...
function useDraggable(target, options = {}) {
  ...
  const position = ref2((_toValue = toValue2(initialValue)) !== null && _toValue !== void 0 ? _toValue : {
    x: 0,
    y: 0
  });
  ...
  const handleEvent = (e) => {
    if (toValue2(preventDefault2)) e.preventDefault();
    if (toValue2(stopPropagation)) e.stopPropagation();
  };
  const scrollConfig = toValue2(autoScroll);
  const scrollSettings = typeof scrollConfig === "object" ? {
    speed: (_toValue2 = toValue2(scrollConfig.speed)) !== null && _toValue2 !== void 0 ? _toValue2 : defaultScrollConfig.speed,
    margin: (_toValue3 = toValue2(scrollConfig.margin)) !== null && _toValue3 !== void 0 ? _toValue3 : defaultScrollConfig.margin,
  ...
  const checkAutoScroll = () => {
    if (toValue2(options.disabled) || !pressedDelta.value) return;
    const container = toValue2(containerElement);
    if (!container) return;
    const targetRect = toValue2(target).getBoundingClientRect();
    const { x, y } = position.value;
  ...
  if (toValue2(autoScroll)) watch2(position, checkAutoScroll);
  const start = (e) => {
    var _container$getBoundin;
    if (!toValue2(buttons).includes(e.button)) return;
    if (toValue2(options.disabled) || !filterEvent(e)) return;
    if (toValue2(exact) && e.target !== toValue2(target)) return;
    const container = toValue2(containerElement);
  ...
  const move = (e) => {
    ...
    if (toValue2(autoScroll) && container) {
...
function useElementByPoint(options) {
  ...
  const isSupported = useSupported(() => {
    if (toValue2(multiple)) return document2 && "elementsFromPoint" in document2;
  ...
  return {
    ...
    ...scheduler(() => {
      ...
      element.value = toValue2(multiple) ? (_document$elementsFro = document2 === null || document2 === void 0 ? void 0 : document2.elementsFromPoint(toValue2(x), toValue2(y))) !== null && _document$elementsFro !== void 0 ? _document$elementsFro : [] : (_document$elementFrom = document2 === null || document2 === void 0 ? void 0 : document2.elementFromPoint(toValue2(x), toValue2(y))) !== null && _document$elementFrom !== void 0 ? _document$elementFrom : null;
    })
  };
}
function useElementHover(el, options = {}) {
  ...
  if (triggerOnRemoval) onElementRemoval(computed2(() => unrefElement(el)), () => toggle(false));
  return isHovered;
}
...
function useIntersectionObserver(target, callback, options = {}) {
  ...
  const targets = computed2(() => {
    return toArray(toValue2(target)).map(unrefElement).filter(notNullish);
  });
  let cleanup = noop;
...
function useElementVisibility(element, options = {}) {
  ...
  const isVisible = shallowRef2(initialValue);
...
function useEventSource(url, events2 = [], options = {}) {
  ...
  const eventSource = shallowRef2(null);
  const error = shallowRef2(null);
  ...
  if (immediate) open();
  if (autoConnect) watch2(urlRef, open);
...
function useFetch(url, ...args) {
  ...
  watch2([refetch, toRef(url)], ([refetch2]) => refetch2 && execute(), { deep: true });
  const shell = {
    isFinished: shallowReadonly2(isFinished),
    isFetching: shallowReadonly2(isFetching),
    statusCode,
    response,
    error,
    data,
  ...
  function setMethod(method) {
    return (payload, payloadType) => {
      if (!isFetching.value) {
        ...
        if (isRef2(config.payload)) watch2([refetch, toRef(config.payload)], ([refetch2]) => refetch2 && execute(), { deep: true });
        return {
          ...shell,
          then(onFulfilled, onRejected) {
...
function useFileDialog(options = {}) {
  ...
  const applyOptions = (options2) => {
    ...
    if (hasOwn(options2, "capture")) el.capture = toValue2(options2.capture);
  };
  const open = (localOptions) => {
    ...
    if (toValue2(mergedOptions.reset)) reset();
    el.click();
  };
  watchEffect2(() => {
...
function useIdle(timeout = oneMinute, options = {}) {
  ...
  return {
    ...
    stop,
    start,
    isPending: shallowReadonly2(isPending)
  };
}
...
function useImage(options, asyncStateOptions = {}) {
  const state = useAsyncState(() => loadImage(toValue2(options)), void 0, {
    resetOnExecute: true,
    ...asyncStateOptions
  });
  watch2(() => toValue2(options), () => state.execute(asyncStateOptions.delay), { deep: true });
  return state;
}
...
function useScroll(element, options = {}) {
  ...
  function scrollTo(_x, _y) {
    ...
    (_ref = _element instanceof Document ? window2.document.body : _element) === null || _ref === void 0 || _ref.scrollTo({
      top: (_toValue = toValue2(_y)) !== null && _toValue !== void 0 ? _toValue : y.value,
      left: (_toValue2 = toValue2(_x)) !== null && _toValue2 !== void 0 ? _toValue2 : x.value,
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  const state = reactive2(useScroll(element, {
    ...options,
    offset: {
...
function useMagicKeys(options = {}) {
  ...
  const proxy = new Proxy(refs, { get(target2, prop, rec) {
    ...
    } else refs[prop] = shallowRef2(false);
...
function usingElRef(source, cb) {
  if (toValue2(source)) cb(toValue2(source));
}
...
function useMediaControls(target, options = {}) {
  ...
  const buffered = shallowRef2([]);
  const tracks = shallowRef2([]);
  ...
  useEventListener(target, "timeupdate", () => ignoreCurrentTimeUpdates(() => currentTime.value = toValue2(target).currentTime), listenerOptions);
  useEventListener(target, "durationchange", () => duration.value = toValue2(target).duration, listenerOptions);
  useEventListener(target, "progress", () => buffered.value = timeRangeToArray(toValue2(target).buffered), listenerOptions);
  ...
  useEventListener(target, "ratechange", () => rate.value = toValue2(target).playbackRate, listenerOptions);
...
function useMouseInElement(target, options = {}) {
  ...
  const targetRef = shallowRef2(target !== null && target !== void 0 ? target : window2 === null || window2 === void 0 ? void 0 : window2.document.body);
...
function useNavigatorLanguage(options = {}) {
  ...
  const language = shallowRef2(navigator2 === null || navigator2 === void 0 ? void 0 : navigator2.language);
...
function useNetwork(options = {}) {
  ...
  return {
    isSupported,
    isOnline: shallowReadonly2(isOnline),
    saveData: shallowReadonly2(saveData),
    offlineAt: shallowReadonly2(offlineAt),
    onlineAt: shallowReadonly2(onlineAt),
    downlink: shallowReadonly2(downlink),
    downlinkMax: shallowReadonly2(downlinkMax),
    effectiveType: shallowReadonly2(effectiveType),
    rtt: shallowReadonly2(rtt),
    type: shallowReadonly2(type)
  };
}
...
function useNow(options = {}) {
  ...
  const now2 = shallowRef2(/* @__PURE__ */ new Date());
...
function useObjectUrl(object) {
  ...
  watch2(() => toValue2(object), (newObject) => {
    release();
    if (newObject) url.value = URL.createObjectURL(newObject);
  }, { immediate: true });
  tryOnScopeDispose(release);
  return shallowReadonly2(url);
}
function useClamp(value, min, max) {
  if (typeof value === "function" || isReadonly(value)) return computed2(() => clamp(toValue2(value), toValue2(min), toValue2(max)));
...
function useOffsetPagination(options) {
  ...
  const pageCount = computed2(() => Math.max(1, Math.ceil(toValue2(total) / toValue2(currentPageSize))));
  const currentPage = useClamp(page, 1, pageCount);
  const isFirstPage = computed2(() => currentPage.value === 1);
  const isLastPage = computed2(() => currentPage.value === pageCount.value);
  if (isRef2(page)) syncRef(page, currentPage, { direction: isReadonly(page) ? "ltr" : "both" });
  if (isRef2(pageSize)) syncRef(pageSize, currentPageSize, { direction: isReadonly(pageSize) ? "ltr" : "both" });
  function prev() {
    currentPage.value--;
  }
  function next() {
...
function useScreenOrientation(options = {}) {
  ...
  const orientation = shallowRef2(screenOrientation.type);
...
function usePointerSwipe(target, options = {}) {
  ...
  return {
    isSwiping: shallowReadonly2(isSwiping),
    direction: shallowReadonly2(direction),
...
function usePreferredLanguages(options = {}) {
  const { window: window2 = defaultWindow } = options;
  if (!window2) return shallowRef2(["en"]);
...
function useSorted(...args) {
  ...
  if (!dirty) return computed2(() => sortFn([...toValue2(source)], compareFn));
  watchEffect2(() => {
    const result = sortFn(toValue2(source), compareFn);
    if (isRef2(source)) source.value = result;
...
function useSpeechSynthesis(text, options = {}) {
  ...
  if (isSupported.value) {
    ...
    if (options.voice) watch2(options.voice, () => {
      synth.cancel();
    });
    watch2(isPlaying, () => {
...
function useStepper(steps, initialStep) {
  ...
  const index = ref2(stepNames.value.indexOf(initialStep !== null && initialStep !== void 0 ? initialStep : stepNames.value[0]));
...
function useStyleTag(css, options = {}) {
  ...
  return {
    ...
    unload,
    load,
    isLoaded: shallowReadonly2(isLoaded)
  };
}
function useSwipe(target, options = {}) {
  const { threshold = 50, onSwipe, onSwipeEnd, onSwipeStart, passive = true } = options;
  const coordsStart = reactive2({
    x: 0,
    y: 0
  });
  const coordsEnd = reactive2({
    x: 0,
    y: 0
  });
...
function useTextDirection(options = {}) {
  ...
  const dir = shallowRef2(getValue2());
...
function useTextSelection(options = {}) {
  ...
  const { window: window2 = defaultWindow } = options;
  const selection = shallowRef2((_window$getSelection = window2 === null || window2 === void 0 ? void 0 : window2.getSelection()) !== null && _window$getSelection !== void 0 ? _window$getSelection : null);
...
function useTextareaAutosize(options = {}) {
  ...
  function triggerResize() {
    ...
    const _styleTarget = toValue2(options === null || options === void 0 ? void 0 : options.styleTarget);
  ...
  if (options === null || options === void 0 ? void 0 : options.watch) watch2(options.watch, triggerResize, {
    immediate: true,
    deep: true
  });
  return {
...
function useTimeAgoIntl(time, options = {}) {
  ...
  const result = computed2(() => getTimeAgoIntlResult(new Date(toValue2(time)), options, toValue2(now2)));
  const parts = computed2(() => result.value.parts);
  const timeAgoIntl = computed2(() => formatTimeAgoIntlParts(parts.value, {
    ...options,
...
function useTitle(newTitle = null, options = {}) {
  ...
  const title = toRef((_ref = newTitle !== null && newTitle !== void 0 ? newTitle : document2 === null || document2 === void 0 ? void 0 : document2.title) !== null && _ref !== void 0 ? _ref : null);
...
function defaultInterpolation(a, b, t) {
  const aVal = toValue2(a);
  const bVal = toValue2(b);
  if (typeof aVal === "number" && typeof bVal === "number") return lerp(aVal, bVal, t);
  if (Array.isArray(aVal) && Array.isArray(bVal)) return aVal.map((v, i) => lerp(v, toValue2(bVal[i]), t));
  throw new TypeError("Unknown transition type, specify an interpolation function.");
}
function normalizeEasing(easing) {
  var _toValue;
  return typeof easing === "function" ? easing : (_toValue = toValue2(easing)) !== null && _toValue !== void 0 ? _toValue : identity;
}
function transition(source, from, to, options = {}) {
  ...
  const duration = (_toValue2 = toValue2(options.duration)) !== null && _toValue2 !== void 0 ? _toValue2 : 1e3;
...
function useTransition(source, options = {}) {
  ...
  const sourceVal = () => {
    ...
    return typeof options.interpolation === "undefined" && Array.isArray(v) ? v.map(toValue2) : v;
  };
  const outputRef = shallowRef2(sourceVal());
  watch2(sourceVal, async (to) => {
    var _options$onStarted, _options$onFinished;
    if (toValue2(options.disabled)) return;
    const id = ++currentId;
    if (options.delay) await promiseTimeout(toValue2(options.delay));
...
function useUrlSearchParams(mode = "history", options = {}) {
  ...
  if (!window2) return reactive2(initialValue);
...
function useUserMedia(options = {}) {
  ...
  const enabled = shallowRef2((_options$enabled = options.enabled) !== null && _options$enabled !== void 0 ? _options$enabled : false);
  const autoSwitch = shallowRef2((_options$autoSwitch = options.autoSwitch) !== null && _options$autoSwitch !== void 0 ? _options$autoSwitch : true);
...
function useWebNotification(options = {}) {
  ...
  const notification = shallowRef2(null);
  const ensurePermissions = async () => {
    if (!isSupported.value) return;
  ...
  const _init = () => {
    ...
    ws.onmessage = (e) => {
      if (options.heartbeat) {
        ...
        if (e.data === toValue2(responseMessage)) return;
      }
      data.value = e.data;
...
function useWindowFocus(options = {}) {
  const { window: window2 = defaultWindow } = options;
  if (!window2) return shallowRef2(false);
...
function useWindowSize(options = {}) {
  ...
  if (listenOrientation) watch2(useMediaQuery("(orientation: portrait)"), () => update());
  return {
    width,
    height
  };
