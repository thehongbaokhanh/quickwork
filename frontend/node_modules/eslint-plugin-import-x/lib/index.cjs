...
function parse(path$21, content, context) {
	...
	parserOptions.filePath = path$21;
...
function getParserOrPath(path$21, context) {
	const parsers = context.settings["import-x/parsers"];
	if (parsers != null) {
		const extension = node_path.default.extname(path$21);
	...
	if ("parserPath" in context && context.parserPath) {
		log$4("using context.parserPath:", context.parserPath);
		return context.parserPath;
	}
	const parser = "languageOptions" in context && context.languageOptions?.parser;
	if (parser && typeof parser !== "string" && ("parse" in parser && typeof parser.parse === "function" || "parseForESLint" in parser && typeof parser.parseForESLint === "function")) return parser;
	return null;
}
...
function isDirectoryAccessError(error) {
...
function fileExistsWithCaseSync(filepath, cacheSettings, strict, leaf = true) {
	...
	if (dir === "" || parsedPath.root === filepath) result = true;
		...
		if (filenames.includes(parsedPath.base)) result = fileExistsWithCaseSync(dir, cacheSettings, strict, false);
		else {
			const baseLowerCase = parsedPath.base.toLowerCase();
			const hasCaseInsensitiveMatch = filenames.some((p) => p.toLowerCase() === baseLowerCase);
			...
			if (isMissing && leaf) {
				...
				result = index > 0 ? fileExistsWithCaseSync(filepath.slice(0, index), cacheSettings, strict) : false;
			} else result = isMissing && !leaf;
		}
	} catch (error) {
		if (isDirectoryAccessError(error)) result = true;
		else throw error;
	}
...
function getCachedNodeResolver(opts) {
	const key = (0, stable_hash_x.stableHash)(opts);
...
function legacyNodeResolve(resolverOptions, context, modulePath, sourceFile) {
	...
	if (normalizedPaths?.length) {
		...
		if (paths$1.length > 0) {
			const resolver$1 = getCachedNodeResolver({
				extensions: normalizedExtensions,
				builtinModules: includeCoreModules !== false,
				modules: paths$1,
				symlinks,
...
var ExportMap = class ExportMap {
	...
	static get(source, context) {
		const path$21 = resolve(source, context);
		if (path$21 == null) return null;
		return ExportMap.for(childContext(path$21, context));
	}
...
const parsePath = (path$21) => {
	const hashIndex = path$21.indexOf("#");
	const queryIndex = path$21.indexOf("?");
	const hasHash = hashIndex > 0;
	const hash = hasHash ? path$21.slice(hashIndex) : "";
	const hasQuery = queryIndex !== -1 && (!hasHash || queryIndex < hashIndex);
	const query = hasQuery ? path$21.slice(queryIndex, hasHash ? hashIndex : void 0) : "";
	const pathname = hasQuery ? path$21.slice(0, queryIndex) : hasHash ? path$21.slice(0, hashIndex) : path$21;
	return {
		pathname,
		query,
		hash
...
function sourceType(context) {
	if ("languageOptions" in context && context.languageOptions) {
		if ("parserOptions" in context.languageOptions && context.languageOptions.parserOptions?.sourceType) return context.languageOptions.parserOptions.sourceType;
		if ("sourceType" in context.languageOptions && context.languageOptions.sourceType) return context.languageOptions.sourceType;
	}
	if ("parserOptions" in context && context.parserOptions) return context.parserOptions.sourceType;
}
...
function computeOverrideAction(pathGroupOverrides, path$21) {
	for (const { pattern: pattern$1, patternOptions, action } of pathGroupOverrides) if ((0, minimatch.minimatch)(path$21, pattern$1, patternOptions || { nocomment: true })) return action;
}
/**
...
var extensions_default = createRule({
	...
	create(context) {
		...
		return moduleVisitor((source, node) => {
			...
			const extension = node_path.default.extname(resolvedPath && dtsRe.test(resolvedPath) ? importPath : resolvedPath || importPath).slice(1);
	...
	create(context) {
		...
		return {
			...
			VariableDeclarator(node) {
				...
				function testKey(pattern$1, namespace, path$21 = [initName]) {
					...
					for (const property of pattern$1.properties) {
						...
						if (!namespace.has(property.key.name)) {
							context.report(makeMessage(property.key, path$21, property));
							continue;
						}
						path$21.push(property.key.name);
						const dependencyExportMap = namespace.get(property.key.name);
						if (dependencyExportMap != null) testKey(property.value, dependencyExportMap.namespace, path$21);
						path$21.pop();
					}
				}
		...
		return {
			...moduleVisitor(function checkSourceValue(sourceNode, importer) {
				...
				function detectCycle({ mget, route }) {
					...
					for (const [path$21, { getter, declarations }] of m.imports) {
						if (traversed.has(path$21)) continue;
						...
						*/
						if (path$21 === filename && toTraverse.length > 0) return true;
...
function isESLint9UnsupportedApi(input) {
	return typeof input === "object" && input !== null && "shouldUseFlatConfig" in input && "FileEnumerator" in eslint_use_at_your_own_risk.default;
}
function ensureESLint9UnsupportedApi(input) {
	if (!isESLint9UnsupportedApi(input)) throw new TypeError("ESLint 10 and later versions remove the FileEnumerator API, which is required by the \"no-unused-modules\" rule.");
	return input;
}
function listFilesUsingFileEnumerator(src, extensions) {
	const { FileEnumerator, shouldUseFlatConfig } = ensureESLint9UnsupportedApi(eslint_use_at_your_own_risk.default);
...
const isNodeModule = (path$21) => /([/\\])(node_modules)\1/.test(path$21);
/**
...
var no_unused_modules_default = createRule({
		...
		schema: [{
			...
			properties: {
				...
				suppressMissingFileEnumeratorAPIWarning: {
					description: "ESLint 10 and later versions remove the FileEnumerator API, which is required by the \"no-unused-modules\" rule. Therefore a warning message will be logged before an alternative is implemented. You can suppress the warning with this option",
					type: "boolean"
				}
			},
			anyOf: [{
	...
	defaultOptions: [],
	create(context) {
		const { src = [process.cwd()], ignoreExports = [], missingExports, unusedExports, ignoreUnusedTypeExports, suppressMissingFileEnumeratorAPIWarning } = context.options[0] || {};
		if (!isESLint9UnsupportedApi(eslint_use_at_your_own_risk.default)) {
...
function computePathRank(ranks, pathGroups, path$21, maxPosition) {
	for (const { pattern: pattern$1, patternOptions, group, position = 1 } of pathGroups) if ((0, minimatch.minimatch)(path$21, pattern$1, patternOptions || { nocomment: true })) return ranks[group] + position / maxPosition;
}
