import nodeModule, { builtinModules, createRequire } from "node:module";
...
import fs$1, { promises, realpathSync, statSync } from "node:fs";
...
//#endregion
//#region node_modules/.pnpm/@eslint+config-array@0.23.5_patch_hash=9c56839266cb75a04e2a96a342456c69afd7549bf6c13a9ade49cc6d42de892a/node_modules/@eslint/config-array/dist/esm/std__path/posix.js
...
//#endregion
//#region node_modules/.pnpm/@eslint+config-array@0.23.5_patch_hash=9c56839266cb75a04e2a96a342456c69afd7549bf6c13a9ade49cc6d42de892a/node_modules/@eslint/config-array/dist/esm/std__path/windows.js
...
minimatch$1.sep = defaultPlatform === "win32" ? path$1.win32.sep : path$1.posix.sep;
...
//#endregion
//#region node_modules/.pnpm/@eslint+object-schema@3.0.5/node_modules/@eslint/object-schema/dist/esm/index.js
	...
	*/
	constructor(definitions) {
		...
		for (const key of Object.keys(definitions)) {
			...
			if (typeof normalizedDefinition.merge === "string") normalizedDefinition = {
				...normalizedDefinition,
				merge: MergeStrategy[normalizedDefinition.merge]
			};
			if (typeof normalizedDefinition.validate === "string") normalizedDefinition = {
				...normalizedDefinition,
				validate: ValidationStrategy[normalizedDefinition.validate]
			};
			this.#definitions.set(key, normalizedDefinition);
			if (normalizedDefinition.required) this.#requiredKeys.set(key, normalizedDefinition);
		}
	}
	/**
...
//#endregion
//#region node_modules/.pnpm/@eslint+config-array@0.23.5_patch_hash=9c56839266cb75a04e2a96a342456c69afd7549bf6c13a9ade49cc6d42de892a/node_modules/@eslint/config-array/dist/esm/index.js
/**
...
function assertExtraConfigTypes(extraConfigTypes) {
	if (!Array.isArray(extraConfigTypes)) throw new TypeError("extraConfigTypes must be an array.");
	if (extraConfigTypes.length > 2) throw new TypeError("extraConfigTypes must contain at most two items.");
	for (const configType of extraConfigTypes) if (!CONFIG_TYPES.has(configType)) throw new TypeError(`Unexpected config type "${configType}" in extraConfigTypes. Expected one of: "array", "function".`);
}
/**
...
*/
function getMatchedGlobs(file, globs) {
	return globs.filter((g) => Array.isArray(g) ? g.every((p) => minimatch(file, p)) : minimatch(file, g));
}
function globEntryKey(g) {
	return Array.isArray(g) ? `[${g.join("\0")}]` : g;
}
...
const depsStorage = new AsyncLocalStorage();
let loadGen = 0;
...
function installResolveHook() {
	...
	nodeModule.registerHooks({ resolve(specifier, context, nextResolve) {
		...
		const path = normalize$3(fileURLToPath(result.url));
		if (!path.startsWith(store.realBasePath)) return result;
		store.deps.add(path);
		let mtimeMs;
		try {
...
async function readConfig(options) {
	...
	if (hookSupported) {
		...
		let realBasePath;
		try {
			realBasePath = normalize$3(realpathSync(basePath));
		} catch {
	...
	const exported = await (mod?.default ?? mod);
	const rawConfigs = Array.isArray(exported) ? [...exported] : [exported];
	rawConfigs.unshift({
		index: 1,
	...
	const configs = rawConfigs.map((c, idx) => {
		...
		if (c.plugins) out.plugins = Object.fromEntries(Object.entries(c.plugins).map(([prefix]) => [prefix, {}]).filter((i) => i[0]));
		else delete out.plugins;
		if (c.languageOptions) out.languageOptions = {
			...c.languageOptions,
			parser: c.languageOptions.parser?.meta?.name
		};
		else delete out.languageOptions;
		const processorName = c.processor?.meta?.name;
		if (processorName) out.processor = processorName;
		else delete out.processor;
		return out;
	});
...
*/
async function collectStaticDependencies(jiti, configPath) {
	const visited = new Set([configPath]);
	...
	while (queue.length) {
		...
		for (const imp of imports) {
			...
			const resolvedPath = normalize$3(resolved.startsWith("file://") ? fileURLToPath(resolved) : resolved);
...
async function globMatchedFiles(basePath, configs, rawConfigs) {
	...
	return [...await configArrayFindFiles(basePath, buildConfigArray(rawConfigs, basePath))].sort().map((filepath) => {
...
*/
async function configArrayFindFiles(basePath, configs) {
	return new Promise((resolvePromise, rejectPromise) => {
		...
		import_out.walk(basePath, {
			...
			entryFilter: safeFilter((entry) => !entry.dirent.isDirectory() && configs.getConfig(entry.path) !== void 0)
		}, (error, entries) => {
			if (error) rejectPromise(error);
			else resolvePromise(entries.map((e) => e.path));
		});
	});
}
//#endregion
...
*/
function setBuildPayload(payload) {
...
const devframe = defineDevframe({
	...
	async setup(ctx, info) {
		...
		async function load() {
			if (ctx.mode === "build") {
				if (buildPayload !== void 0) return buildPayload;
			}
			try {
				...
				if (ctx.mode === "dev") {
					if (!watcher) {
						...
						watcher.on("change", (path) => {
							...
							ctx.rpc.broadcast({
								method: "eslint-config-inspector:invalidate",
								args: [path]
							});
						});
					}
		...
		ctx.rpc.register(defineRpcFunction({
			...
			type: "query",
			snapshot: true,
			handler: async () => payload ?? await load()
		}));
	}
});
//#endregion
//#region src/build-wrapper.ts
async function runBuild(options) {
	...
	if (baseURL !== "/") {
		...
		for (const file of htmlFiles) {
			const filepath = resolve$3(outDir, file);
			const newContent = (await fs.readFile(filepath, "utf-8")).replaceAll(/\s(href|src)="\//g, ` $1="${baseURL}`).replaceAll("baseURL:\"/\"", `baseURL:"${baseURL}"`).replaceAll("\"#entry\":\"/_nuxt/", `"#entry":"${baseURL}_nuxt/`);
			await fs.writeFile(filepath, newContent, "utf-8");
		}
	}
	console.log(MARK_CHECK, `Built to ${relative$2(cwd, outDir)}`);
	console.log(MARK_INFO, `You can use a static server like \`npx serve ${relative$2(cwd, outDir)}\` to serve the inspector`);
}
//#endregion
//#region src/cli.ts
const cli = cac("eslint-config-inspector");
cli.command("build", "Build inspector with current config file for static hosting").option("--config <configFile>", "Config file path").option("--files", "Include matched file paths in payload", { default: true }).option("--basePath <basePath>", "Base directory for globs to resolve. Default to directory of config file if not provided").option("--base <baseURL>", "Base URL for deployment", { default: "/" }).option("--outDir <dir>", "Output directory", { default: "dist/__eslint-config-inspector" }).action(async (options) => {
...
cli.command("mcp", "Start an MCP server exposing the inspector to coding agents (stdio) [experimental]").action(async () => {
...
cli.command("", "Start dev inspector").option("--config <configFile>", "Config file path").option("--files", "Include matched file paths in payload", { default: true }).option("--basePath <basePath>", "Base directory for globs to resolve. Default to directory of config file if not provided").option("--host <host>", "Host", { default: process$1.env.HOST || "127.0.0.1" }).option("--port <port>", "Port", { default: Number(process$1.env.PORT) || 7777 }).option("--open", "Open browser", { default: true }).option("--no-open", "Do not open browser").action(async (options) => {
	...
	await createDevServer(devframe, {
		host,
		port: await resolveDevServerPort(devframe, {
			host,
