...
import { getConversationError, mergeConversationMessages, sortConversations } from '~/utils/conversation';
export function useConversationChat(options) {
    ...
    const canSendMessages = computed(() => Boolean(selectedConversation.value
    ...
    async function loadMessages(conversationID = selectedConversationID.value, loadOptions = {}) {
        if (!conversationID)
            return false;
        try {
            if (!loadOptions.silent)
    ...
    async function loadOlderMessages() {
        ...
        if (!conversationID || !hasMoreMessages.value || !nextBeforeID.value || loadingOlder.value)
            return false;
        try {
    ...
    async function markRead(conversationID = selectedConversationID.value) {
        ...
        try {
            ...
            messages.value = messages.value.map(message => (String(message.sender_id) === String(currentUserID.value)
                ? message
    ...
    async function poll() {
        if (import.meta.client && document.hidden)
            return;
        if (conversationsLoading.value || messagesLoading.value || loadingOlder.value || sending.value || backgroundRefreshing.value)
            return;
        await loadConversations({ silent: true, preserveSelection: true });
